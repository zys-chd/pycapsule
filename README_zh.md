...
```
PyInstaller:  74 MB  ──→  pycapsule:  < 1 MB   （缩小 100 倍）
```
...
```bash
cd your-project
git clone https://github.com/zys-chd/pycapsule.git
```

### 2. 编辑配置

打开 `pycapsule/src/config.h`，改 5 个地方：
...
在项目根目录创建 `bootstrap.py`（参考 `pycapsule/example/bootstrap.py`）：
...
# → pycapsule/my_tool (macOS/Linux) 或 pycapsule/my_tool.exe (Windows)
```
...
**不想装 zlib？** 下载 [miniz.h](https://github.com/richgel999/miniz) 放到 `pycapsule/src/`，  
...
在项目根目录创建 `pycapsule-exclude.txt`：
...
```
your-project/
├── pycapsule/                 ← git clone 的 pycapsule
│   ├── pack.py            ← 打包脚本
│   └── src/
│       ├── config.h       ← ✏️ 你的配置
│       └── launcher.c     ← C 源码（不用改）
├── bootstrap.py           ← ✏️ 你的启动入口
├── pycapsule-exclude.txt      ← （可选）排除列表
