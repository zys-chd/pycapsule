#!/usr/bin/env python3
"""
pybox — Python 项目打包为独立 C 二进制

用法:
    cd your-project
    python pybox/pack.py               # 完整打包 + 编译
    python pybox/pack.py --zip-only    # 仅生成 resource.h
    python pybox/pack.py --compile     # 仅编译（需要已有 resource.h）
...
移植步骤:
    1. git clone pybox 放到项目根目录
    2. 编辑 pybox/src/config.h
    3. 创建 bootstrap.py（项目启动入口）
    4. python pybox/pack.py
"""
...
# ── 排除列表（用户可在项目根目录放 pybox-exclude.txt 覆盖）────
EXCLUDE_DIRS = {
...
# 用户自定义排除文件
_usr_exclude = PROJECT_ROOT / "pybox-exclude.txt"
...
def read_config():
    ...
    if not config_h.exists():
        print("[错误] 找不到 pybox/src/config.h")
        print("请从 pybox/src/config.h 模板复制并编辑。")
        sys.exit(1)
...
def generate_header(zip_data: bytes):
    ...
    lines = [
        "/* auto-generated by pybox pack.py */",
...
# ── 主流程 ────────────────────────────────────────────
def pack():
    print(f"pybox — {PROJECT_ROOT.name}")
    print()
    ...
    if 'PROJECT_NAME        "模型拟合工具"' in config_text:
        ...
        print("    请编辑 pybox/src/config.h 后再运行。\n")
        print("    需要修改的字段：")
    ...
    ext = ".exe" if sys.platform == "win32" else ""
    print(f"\n[✓] 完成: pybox/{OUTPUT_NAME}{ext}")
    return 0


if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="pybox — Python project packager")
